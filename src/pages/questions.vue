  ...
  <q-page id="question">
    ...
    <q-tab-panels v-model="tab" animated class="tab-panels flex m-auto w-85">
      <q-tab-panel name="all" class="flex row justify-betwen" >
        <q-btn align="left" class="flex col-xs-12 col-sm-6 col-lg-4 qus_btn" v-for="item in discuss" :key="item._id" :value="item" @click="discussOpen(item)">
          <div class="border">
        ...
        <q-card-section>
          <div class="fb" style="" v-for="fb in discussmodel.feedback" :key="fb._id" :value="fb">
            <div class="flex row align-center h-auto">
              ...
              <div class="col-2 fb-1">{{ fb.user }}</div>
  ...
  methods: {
    ...
    onSubmit () {
      if (this.user.account === '') {
        ...
        this.axios.post(process.env.VUE_APP_API + '/discuss/', reply)
          .then(res => {
            if (res.data.success) {
